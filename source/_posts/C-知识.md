...
- 【类指针的`const`对象和`const`指针】 `const`对象`const Box* p;`不能通过`p`修改对象,但是可以改变指向 ; `const`指针`Box* const p=&a;`这个是指针地址不能变,但对象可以改 ; 两个都`const`: `const Box* const p=&a;`不能改对象也不能改指针指向
