...
- 【内联函数`inline`】是建议编译器将函数展开到调用处(大概是代码直接替换),减少函
数调用开销,不过现代C++不是强制展开,而是编译器自行决定 ; 所有在类内写了定义的成员函数默认就是`inline`的,因为`inline`函数允许多个定义,只要内容相同,所以就算在`.h`头文件中定义了函数也能避免重复定义错误 ; 现代C++一般不需要手动使用`inline`,只有需要`.h`定义或特别小的函数(10行以内)希望优化才加`inline`,并且推荐`inline`替换`#define`,因为有类型检查
