...
出现`git pull`的时候`Please, commit your changes or stash them before you can merge.`:
若要保留本地修改,先`git stash`将工作区恢复到上次提交的内容,同时备份本地所做的修改,然后就可以正常`git pull`了,`git pull`完成后，执行`git stash pop`将之前本地做的修改应用到当前工作区
`git stash clear`:清空Git栈,就是把stash那些节点清理掉
若要放弃本地修改,就`git reset --hard`,然后`git pull`
...
## 迁移git到新目录(hexo也保持正常)
直接复制所有东西到新目录下就行,Git仓库本身不依赖具体的绝对路径,`.git`文件夹跟着一起移动过去,仓库的所有历史、分支、远程关联都会完好保留 -> 但是注意`.git`是隐藏文件夹,记得显示隐藏再复制
使用命令`git status`能正常显示仓库状态、`git remote -v`还指向你的GitHub仓库地址,说明Git迁移成功了
...
## 荔枝派3A、荔枝派4A
【3A】默认用户名root,pw bianbu
【4A】用户名debian,pw debian -> root的pw也是debian,但是无法用root登录
...
## 同up讲内存和显存的区别[这个](https://www.bilibili.com/video/BV1SGXsYxESV/?spm_id_from=333.1387.upload.video_card.click&vd_source=caea238fdc1c97555bce64a9b94b987e)
- 内存条为CPU服务,CPU运算需要的数据和运算产生的结果都会暂存在内存里边
- 围绕在GPU核心周围一圈的就是显存颗粒,和内存功能类似,弄GPU的数据
- DDR标准制造显存不再能满足GPU的需求了 -> 变成GDDR标准
- 【CPU和GPU区别】CPU有数量较少的核心但是每个核心都很强大 ; GPU拥有数量较多的核心但是每个核心都相对弱小 -> GPU适合处理并行任务,由于超高并发,因此显存对带宽的要求远高于内存,增加传输线的个数并降低数据传输之间的间隔
- 时钟信号周期,一个时钟周期内,电信号会抬高然后降低,如果按传统的只在时钟信号由低向上抬的过程中的时候(上升边沿)进行一次传输,就是一个时钟周期内只传输一次,这种电路就是SRD ; 如果在上升边沿和下降边沿都进行一次传输,则一个时钟周期内传输两次,这种电路就是DDR,也就是DDR内存名字的来源
- 但是GDDR5已经将原始时钟信号拷贝4次,然后对这4个时钟信号分别进行四分之一个周期的偏移,然后在每个信号的上边沿触发一次传输,这下一个时钟周期内就可以进行4次传输了 ; 然后GDDR6变成一个时钟周期内8次传输 -> 还是满足不了GPU
- 但是内存没咋提升带宽,因为真的没用 ; 因为GPU处理的每一个任务之间关联不大,但是CPU很多时候都得线性执行,后一步要依靠前一步的结果,而每一次运算需要的数据并不大,所以CPU对内存带宽的要求并不是很高,想提升CPU性能只能让每一步的运算速度变快,所以CPU在卷频率 ; GPU收益最大的是堆规模堆核心
- 对CPU来说,内存的延迟最重要,就是处理器开始进行数据访问到实际数据被访问到的时间差,一般内存的延迟在五六十、七八十纳秒左右,但这样对CPU还是慢,所以CPU一直在加L1、L2、L3缓存就是为了尽可能减少内存的访问 ; 但是延迟对于GPU就没有那么重要了,显存延迟普遍都在200纳秒以上,上面的提高带宽方式很多也是牺牲延迟换来的
...
## 发代码千万别删`output/`目录但要删`/rhohammer/build`,还有一些rhohammer参数
- `output/`是逆向出来的地址,这个就用现有的那个千万别改 -> 这个`mem_config.json`在代码中只有读取，没有任何写入，所以保留
...
- 可以让GPT6再生成一些东西在服务器上跑一下验证一下地址结果，然后直接写`mem_config.json`，注意同目录下还有对应的矩阵`generate_matrix-v3.log`这个也要留下别删!!

## 关于运行参数`--dimm-id 1`
- 经GPT检查,这条命令没有指定只使用一个通道。`--dimm-id 1`是实验记录编号，不是选择第 1 根内存条 ; 项目通过 `mmap` 分配 1 GiB 大页，指定 `HUGEPAGE_NUM` 时筛选的是物理大页编号，没有通道筛选。你之前提供的是 Channel A、Channel B 各一根 8 GB 内存。这种等容量双通道布局支持交错分布，连续物理地址可以分布在两根内存上，不能按“这块大页属于某根内存条”理解(根据Intel数据手册)
- 当前没有通道筛选的全局地址映射，应按两个通道的 32 个 bank 建模。命令里的 --geometry 1,4,4 仍然保留，不需要为了表示双通道而改成 2,4,4。
- 只有下面两种情况才能按单通道的 16 个处理： 1.实际只启用一个通道，例如只安装一根内存。 2.已恢复并验证通道选择函数，而且 reverse 的采样地址也明确限制在同一个通道。
- 即便以后 rhoHammer 做了单通道筛选，只要 reverse 仍然从两个通道取样，reverse 仍应按 32 配置。当前候选 JSON 的 SC 是逻辑选择位，也不能直接当作真实通道编号来筛选
- 对当前双通道、未筛选地址的情况，建议在 `reverse/config.ini`中使用：
```
NUM_OF_READ 50
...
## 路zk关于half-double实现理解
- 看了论文,half-double不是将+1行频率减半,+1行根本就不填槽,而是高频访问+2行,利用TRR刷新+1行实现+1行的低频访问,再影响受害行
- 也就是说把double-side的组扩大成5行一组,用更高点的高频访问+2行即可,+1行通过利用TRR得到低频激活

## 260921修一个评估报告无法生成的bug
- 头文件中的`static`全局变量会导致跨文件状态不共享,即变量只属于当前编译单元,一个`.cpp`连同它包含的头文件,经过预处理后构成一个编译单元,如果头文件被多个`.cpp`包含,就相当于把这行定义分别放进了各个编译单元,它们虽然名字相同,但实际上是地址不同、彼此独立的两个对象,因为具有内部链接,它们也不会被当成重复定义报错
- 原本的`util.h`中写的是`static std::atomic<bool> running{true};`,但是这个头文件被`sequent_rowhammer_sync.cpp`和`util.cc`分别包含了,虽然这两个文件都能用`running`,但实际上是两个不同对象
- 卡住原因: 主线程执行`running.store(false); t.join();`,但监控线程执行的是`util.cc`中的函数有一个`while(running.load())`,于是主程序把 `running_A`改成 `false` -> 监控线程仍读取 `running_B`,它始终为`true` -> 监控线程一直循环 -> `t.join()`一直等待监控线程结束 -> 后面的报告生成代码无法执行
- 【正确写法】头文件声明,其中一个源文件定义: 在`util.h`中改成`extern std::atomic<bool> running;`,然后`util.cc`在`#include "util.h"`之后定义`std::atomic<bool> running{true};`,`extern`这一行只是声明: "程序中存在这个变量，定义在别处"; 这样提供唯一的定义后,主程序和监控线程就会访问同一个对象

## 260924组会上关于新板子指令探测
- 目标平台缓存指令是根据手册得到的,不可能直接通过硬件探测得到指令 -> 【代ht】这玩意做不了,啥来了都不好使,谁知道人家厂里咋定义的嘛 -> 要是真能行,就不用搞RV了,直接去破解英伟达显卡和英特尔/AMD CPU
- 还有RISC-H在RV服务器上有了但是组里达不到? -> RISC-H人家用的是FPGA,甚至可以不要缓存,把CPU和内存挨得很近,还有SPL可定制改的(还有操作系统和驱动适配之类),所以延迟很低才40,但是这边算能服务器光是过3级缓存就不止40了,并且对交替访问之类的也没有优化,所以200延迟导致攻击不可达是设备差异问题
...
- 【`static`】`static`局部变量会导致用起来像全局变量,生命周期变长,但作用域不变 ; 全局变量默认`extern`,使用`static`全局变量可限制该变量作用域为该`cpp`文件,即外部不可见 ; `static`类即类唯一+按类的生命周期来管,而不是对象了 ; 类的`static`成员变量由该类的所有对象共享,不属于某一个对象,也不随某个对象的构造、析构而创建、销毁,类的`static`成员函数则没有`this`指针
...
- 【更多数据结构】
  ...
  - 【哈希表】`unordered_map<string,int> hashTable;`,插入`hashTable["apple"]=10;`,查找`auto it=hashTable.find("apple")`然后判断`if(it!= hashTable.end())`,然后要取值就是`it->second`,或者直接`hashTable.contains(要查的)`,不推荐用`[]`查询,因为这样不存在会自动创建元素 ; 要遍历就是`for(auto it=hashTable.begin();it!=hashTable.end();++it)`然后里面使用`it->second`来访问value ; 删除`.erase(x)` -> 【*注意*】如果是迭代器使用`auto it = mp.begin();`,那用的时候需要`it->first , it->second`,但是如果用`mp[mid]`,`[]`已经根据`key`把`value`取出来了
  ...
  - 【`pair`】需要`#include<utility>`,定义`pair<int, int> p = {10, 3};`,里面就两个东西,`p.first`(为10),`p.second`(为3)【是成员不是函数没有括号!!】,两个类型不要求相同,如`pair<string, int> p = {"apple", 10};`,`map`里的元素本质上也和 `pair` 密切相关; `pair` 既可以 `==`,还可以比较大小,如`pair<int,int> a = {10, 2}; pair<int,int> b = {20, 1}; cout << (a < b); // true`,比较规则基本就是字典序,先比较`first`,如果`first`不一样,结果已经确定 ; 只有`first`相同,才比较`second`,因为可以比较就可以直接放进`priority_queue`里获得大顶堆
...
- 【拷贝构造函数】直接`A a; A b=a;`的话,实际是等价于`A b(a);`,是要调用拷贝构造函数的,但如果没自己写拷贝构造函数,那就会调用编译器默认写的拷贝构造函数,即浅拷贝全复制 -> 但如果类里有指针变量并用`new`申请了资源,析构函数里还有`delete`,那直接挨个浅拷贝的话会在这些对象析构时把一块内存释放2次,所以要自己写拷贝构造函数`ClassName(const ClassName& other){}`,在里面把该`new`的进行深拷贝
- 【友元friend】允许指定的外部函数/类访问当前类的`private`和`protected`成员,在想要开放的类里写上函数且前面加上`friend`即可:`friend void func();`,还有友元类,在要开放的类里加上`friend class ClassTwo;`则`ClassTwo`对该类就有完全访问权限 -> 注意不是属于,只是获得访问权限 -> 注意`friend`会降低封装性,只推荐"精准放行",不推荐大量使用 -> 还有`friend`是单向且不传递的,没有`this`指针
- 【内联函数`inline`】是建议编译器将函数展开到调用处(大概是代码直接替换),减少函数调用开销,不过现代C++不是强制展开,而是编译器自行决定 ; 所有在类内写了定义的成员函数默认就是`inline`的,因为`inline`函数允许多个定义,只要内容相同,所以就算在`.h`头文件中定义了函数也能避免重复定义错误 ; 现代C++一般不需要手动使用`inline`,只有需要`.h`定义或特别小的函数(10行以内)希望优化才加`inline`,并且推荐`inline`替换`#define`,因为有类型检查
- 【类指针的`const`对象和`const`指针】 `const`对象`const Box* p;`不能通过`p`修改对象,但是可以改变指向 ; `const`指针`Box* const p=&a;`这个是指针地址不能变,但对象可以改 ; 两个都`const`: `const Box* const p=&a;`不能改对象也不能改指针指向 -> *即*指针里加的`const`只影响紧跟它的那个东西,紧跟的是类型就只影响不能改值,紧跟的是指针变量就只影响不能变指向
- 【类指针和*多态*】经常是`父类指针=子类对象`,然后父类的函数是虚函数`virtual`,然后在子类里重写这个函数,上面的调用方法就会使用子类的定义,即多态 -> **为什么基类析构函数经常写`virtual`?**:因为如果析构函数不是`virtual`,则可能只调用父类析构而没有调用子类析构
- 【类指针现代C++智能指针】以前是`Box* p=new Box(); delete p;`,现在是`auto p=std::make_unique<Box>();`,自动释放 ; 工程代码里资源管理一般交给:`unique_ptr`,`shared_ptr`,即推荐使用智能指针代替裸`new/delete`进行资源管理
- 【类的静态成员/静态成员函数`static`】变量前面加上`static`后,无论创建多少个类的对象,静态成员都只有一个副本,即使在类对象不存在的情况下也能用`类名::`的方式访问对应的静态成员变量,并且`static`成员变量在C++17后才支持类内初始化,【*生命周期*】普通成员变量是对象创建则存在,对象销毁即释放,静态成员变量是程序启动时创建,程序结束才销毁(可用于对象计数器这种全局共享) ; 静态成员函数,也是加上`static`且类对象不存在的情况下也能用`类名::`的方式调用,静态成员函数没有`this`指针,只能访问静态成员(有`static`的)
- 【类的继承】一般是`class 派生类: public 基类 {};`,一个类可以派生自多个类 ; 我们几乎不使用`protected`或`private`继承,通常使用`public`继承,`private`继承一般改成包含一个对应基类的`private`变量 ; 多继承的写法是`class <派生类名>:<继承方式1> <基类名1>,<继承方式2> <基类名2>,… {};`,不过多继承若出现菱形继承会有二义性冲突问题,解决方法是在两个中间类(如果是多个中间类,也都得全上虚继承才能解决二义问题)都用虚继承,如`class Dog:virtual public Animal {};  class Cat:virtual public Animal{}; `,这里被`virtual`继承的那个基类才叫虚基类(这里是`Animal`),并且虚继承可以完美解决一个菱形冲突,没有传导性,比如菱形最上面的基类如果还是派生自一个更基类,这个继承不需要`virtual`也能随着基类只保留一份
- 【继承的构造析构顺序】: 构造是先调用基类构造函数再调用派生类构造函数(从上往下搭建),析构是先调用派生类析构函数再调用基类析构函数(从下往上拆)(同时基类析构函数通常`virtual`,避免资源泄露 -> 因为假如使用多态`父类指针=子类对象`,这时`delete 父类`有可能会没被识别到这是个子类而直接调用父类的析构函数,而父类析构加上`virtual`就能强制往下找到派生类的析构函数)
- 【重写父类虚函数声明`override`】比如派生类的函数是`void func() override;`,这个`override`就是明确告诉编译器这个函数是重写父类虚函数,如果函数名写错会报错,比如父类是`virtual void show();`而子类误写成了`void shoW() override;`(写成了大写)就会编译直接报错,没有`override`的话就可能悄悄变成另一个函数
- 【*报错*:浮点异常`SIGFPE`】在一个纯`int`运算环境下如果出现了报错"浮点异常"`SIGFPE`,原因一般不是真正的浮点问题,而是整数除零/取模为零 ; 常见原因是区间/参数的上下界发生倒挂(比如`min > max`)导致无符号数运算下溢或环绕,最终在库内部(如`std::uniform_int_distribution`的拒绝采样计算)产生了除数为0的情况
- 【函数重载】在同一作用域同名函数,但参数列表和实现不同 ; 重载的决策:返回值不能区分重载(多个重载如果都匹配,会报二义性错误) ; 对于值传递,加`const`无法进行重载区分 , 而对于引用传递/指针传递和右值引用,加入`const`可以区分,可以重载成另一个函数
- 【运算符重载】是带有特殊名称的函数,由关键字`operator`和其后要重载的运算符符号构成,也有返回类型和参数列表,比如要写成成员函数:`Box operator+(const Box&);` ; 要写成非成员函数还要直接读对象的`private`数据:`friend Box operator+(const Box&, const Box&);` ; 如果有`getter`和`setter`或者要读的东西是`public`,写成非成员函数也可以`Box operator+(const Box&, const Box&);`,然后用`getter`和`setter`读写数据 -> 是改变对象自身的运算符推荐成员函数(如`=`和`[]`),是两个对象运算比较的推荐非成员函数(如`+`和`==`) ; 【*注*】下面这些运算符不能重载: `.`成员访问,`.*`成员指针访问,`::`作用域解析,`sizeof`大小,`?:`三目运算符,`#`预处理符号,`typeid`运行时类型信息 ; 其他大部分都能重载,并且`=`, `[]`, `()`, `->`, `->*`这几个重载必须作为成员函数 , `<<`和`>>`这俩重载(输入输出重载)必须写成友元非成员函数,第一个对象写成`istream`和`ostream`的引用
- 【多态+纯虚函数抽象类+多态实现虚函数表和虚函数指针+对象切片】C++多态允许使用基类**指针或引用**来调用子类的重写方法,从而使得同一接口可以表现不同的行为(只有指针/引用调用虚函数才会触发多态,如果是`.`的话不会触发多态) ; 就是基类虚函数`virtual`,子类重写,调用虚函数时根据对象的实际类型来决定调用哪个版本的函数 ; 纯虚函数:一个包含纯虚函数(虚函数写上`=0`)的类被称为抽象类(Abstract Class,和Java的显式抽象类关键字不同,C++是看是否存在纯虚函数决定类是否是抽象类),纯虚函数使该类不能被直接实例化,纯虚函数没有函数体,声明时使用`=0`,并且它强制派生类提供具体的实现(只有虚函数`virtual`是提供默认实现+类可实例化+子类可选重写,加了`=0`就是不提供默认实现+类不可实例化+子类必须重写) ; 多态的实现是虚函数表(`V-Table`):每个包含虚函数的类都有一个虚函数表,表中存储该类虚函数对应的函数地址 -> 即每个类维护一张它的所有虚函数的表,然后通过一个虚函数指针来从表中查找调用,调用会增加点开销 ; 虚函数指针(`V-Ptr`):对象中包含一个指向该类虚函数表的指针 ; 【*注*】只有通过基类的指针或引用调用虚函数时,才会发生多态 ; 如果直接使用派生类的对象调用函数,那么调用的是派生类中的版本,而不是基类中的版本 ; 具有多态行为的基类析构函数应该声明为`virtual`以确保在删除派生类对象时调用派生类的析构函数,防止资源泄漏 ; 【对象切片】如果是把子类直接赋值给基类，则只有基类部分得到复制，派生的部分丢失 -> 所以这个操作一般用指针类操作(引用或传指针)来触发基类多态,从而避免对象切片
...
- 【命名空间`namespace`】用于给变量、函数、类等划分作用域,主要避免大型项目/不同库之间名字冲突 ; 定义`namespace A { ... }`,访问其中内容用作用域解析运算符`A::xxx`,如标准库本身就在`std`命名空间,所以有`std::cout`、`std::vector` ; 可以`using namespace A;`直接引入整个命名空间，也可以`using A::xxx;`只引入某个名字，正式工程尤其头文件中不推荐`using namespace std;`以避免命名污染 ; 同一命名空间可以分散定义在多个位置/文件中(即同名`namespace`可以多次打开继续往里面加东西)，也可以嵌套(访问按嵌套顺序来,如`company::network::connect();`) -> 【注意】`::`是作用域解析运算符，既可以用于命名空间`std::cout`也可以用于类`Box::func()`
- 【**C++模板**】模板是泛型编程的基础,即以独立于具体数据类型的方式编写代码 ; 函数模板:`template<typename T> 返回类型 函数名(参数表){}`,`T`是类型占位符且名称可以任意写(不过一般写成这个),`typename`也可写成`class`,函数调用时通常可根据实参自动推导`T`,也可显式`func<int>()`指定 ; 类模板:`template<typename T> class 类名{}`,使用时如`类名<int>`指定`T`为`int`,`vector<int>`、`map<string,int>`等STL容器本质上就是类模板 ; 可定义多个模板参数,如`template<typename T,typename U>` ; 模板参数也不一定是类型,也可以是值,如`template<typename T,int N>` -> `Array<int,10>` ; 模板通常需要让使用处看到完整定义,因此一般把模板实现直接写在`.h`头文件中
...
- 【最小覆盖子串】找子串如果要在循环里频繁`substr(pos,len)`的话,就把循环里变成**记录最佳的起始位置和长度**,最后出循环`return`时统一`substr`,这样还可以通过初始化`pos=-1`最后判断`pos`是否为-1来判断是否存在满足要求的答案 -> 再进一步就是优化掉`check()`,用一个`num`变量记录当前窗口已经达到 t 要求数量的字符种类数,这个只在字符计数跨过"满足/不满足"临界点时修改,即`++mps[s[r]]`后`mps[s[r]] == mpt[s[r]]`才`num+1`避免早已满足的字符的重复计算,`--mps[s[l]]`后`mps[s[l]]==mpt[s[l]]-1`才`num-1`避免早已不满足的字符的重复减少
- 【最大子数组和】光用全局最大前缀和减全局最小前缀和是错的,因为还得要最大前缀和对应的位置,必须在最小前缀和对应位置的后面 -> 所以需要边遍历,边维护前面出现过的最小前缀和,必须是后减前才是合法子数组! -> 所以变成枚举当前前缀和作为右端点,用它减去"它之前出现过的最小前缀和",再对所有右端点取最大值
- 【写`sort`的`cmp`函数】`cmp(a, b)` 返回 `true` 的真正含义是：我希望此时 a 排在 b 前面 -> 即为`true`表示第一个参数应该放前面,`false`表示第一个参数不应放前面
- 【减少边界bug】访问 `a[i+1]` 时，把边界检查直接放在访问之前，如 `while(i+1<n && ...)`,利用 `&&`短路避免越界; 更进一步,优先设计"不需要看下一个元素"的循环,例如遍历当前元素并与`ans.back()`比较。与其靠多个 `if/break`修补边界,不如让边界情况自然落入普通控制流