...
- 【友元friend】允许指定的外部函数/类访问当前类的`private`和`protected`成员,在想要开放的类里写上函数且前面加上`friend`即可:`friend void func();`,还有友元类,在要开放的类里加上`friend class ClassTwo;`则`ClassTwo`对该类就有完全访问权限 -> 注意不是属于,只是获得访问权限 -> 注意`friend`会降低封装性,只推荐"精准放行",不推荐大量使用 -> 还有`friend`是单向且不传递的