...
> 【启动项管理】:任务管理器左边"启动应用"这里,把不需要开机就自动运行的软件全部右键选择"禁用" -> 建议禁用"手机连接"和"移动设备",避免开机时这俩进行局域网搜索手机进行握手同步产生卡顿 -> 不需要的软件开机自启动禁用 -> 【*注意*】火绒里那个`GamingCenterTray , GamingCenter3_Cros.UWP`是机哥电竞控制台,开着吧
> 【限制坦克世界帧数,避免GPU过载】打开NVIDIA控制面板,左边第二项"管理3D设置",上面"程序设置","选择要自定义的程序"这里选中坦克世界那个(World of Tanks,`worldoftanks.exe`),下面那个"指定该程序的设置值"表格往下翻,找到"最大帧速率"这一项,改成"100每秒帧数"
